...
export default function RecentBlog() {
  ...
  return (
    <main>
      <section className="w-[100%] h-[1000px] bg-[#0A0A0A] mt-36 relative  ">
...

            {
              blogs.map((blog) => <SwiperSlide key={blog.name}>
                <div className="text-white">
                  <div className="mx-auto mt-36 mb-14 max-w-[380px] space-y-6 rounded-xl bg-[#ffffff]  pb-8   shadow-lg bg-transparent border border-[#F2F3F6]">
                    ...
                    <div className="mx-auto w-[85%] space-y-2 text-left font-bold">
                      <h6 className="text-[24px] leading-[31px]">{blog.title}</h6>
                    </div>
                    <div className="flex flex-wrap items-center  justify-between w-[85%] mx-auto">
                      <button className="duration-300 hover:scale-105 flex items-center gap-2 text-[#55E6A5]">Read More <GoArrowUpRight /></button>
                      <button className="bg-[#C8CFD5] text-[#18191C] font-bold text-[14px] leading-[18px] rounded-[6px] px-[10px] py-[15px] ">
                        <p>{blog.date}</p>
                      </button>
                    </div>
                  </div>
